...
function registerBubbleRegex() {
    ...
    try {
        ...
        const regexScript = {
            id: REGEX_SCRIPT_ID,
            scriptName: '对话渲染v7.1',
            findRegex: String.raw`/<now_plot>([\s\S]*?)<\/now_plot>/gi`,
            replaceString: `\`\`\`\`html
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
\`\`\`\``,
        ...
        saveSettingsDebounced();
        console.log('[BubbleDialogue] 对话气泡正则脚本已注册');
    } catch (e) {
        console.warn('[BubbleDialogue] 正则脚本注册失败:', e);
    }
}
...
/**
 * ST extension activate hook - called by the extension system.
 * Replaces the original jQuery $(() => { ... }) ready block.
 */
export function init() {
    console.log('[BubbleDialogue] ST-BubbleDialogue v8.0.0 初始化...');

    // Original init sequence (extracted from jQuery ready block)
...
// Pagehide cleanup (was $(window).on('pagehide', ...))
window.addEventListener('pagehide', () => { avatarDB.revokeAllUrls(); });
